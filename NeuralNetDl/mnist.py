...
def neural_network_model(data):
	...
	l1 = tf.matmul(data,hidden_1_layer['weight']) + hidden_1_layer['baises']
	l1 = tf.nn.relu(l1)

	l2 = tf.matmul(l1,hidden_2_layer['weight']) + hidden_2_layer['baises']
	l2 = tf.nn.relu(l2)
	
	l3 = tf.matmul(l2,hidden_3_layer['weight']) + hidden_3_layer['baises']
	l3 = tf.nn.relu(l3)
...
def train_neural_network(x):
	...
	with tf.Session() as sess:
		...
		for epoch in range(hm_epoch):
			...
			print('Epoc',epoch,'completed out of',hm_epoch,'loss',epoch_loss)

		correct = tf.equal(tf.argmax(prediction,1),tf.argmax(y,1))
		accuracy = tf.reduce_mean(tf.cast(correct,'float'))
		print('Accuracy',accuracy.eval({x:mnist.text.image,y:mnist.test.labels}))
